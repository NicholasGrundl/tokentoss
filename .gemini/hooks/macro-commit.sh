...
print_session_data() {
    ...
    local staged_diff=$(git diff --staged -- ':!.gemini/micro_commits.log' ':!.gemini/last_macro_commit' ':!.gemini/macro_amend.lock' 2>/dev/null)
    local git_diff="$hist_diff\n$staged_diff"

    local hist_changed_files=$(git diff --name-only "$diff_range" 2>/dev/null | grep -vE "^\.gemini/(micro_commits\.log|last_macro_commit|macro_amend\.lock)")
    local staged_changed_files=$(git diff --name-only --staged 2>/dev/null | grep -vE "^\.gemini/(micro_commits\.log|last_macro_commit|macro_amend\.lock)")
